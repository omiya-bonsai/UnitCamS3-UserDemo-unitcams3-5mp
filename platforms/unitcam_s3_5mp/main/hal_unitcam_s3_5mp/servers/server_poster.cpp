...
void HAL_UnitCamS3_5MP::startPosterServer()
{
    ...
    // Start local web/API server instead of cloud poster.
    // This keeps the device on the home Wi-Fi (STA) and exposes the same APIs
    // such as /api/v1/capture and /api/v1/stream for Home Assistant.
    spdlog::info("start local web server instead of poster task");
    // startApServer();
    startLocalWebServer();

    // Cloud poster disabled for local Home Assistant use.
    // start_poster_task();
